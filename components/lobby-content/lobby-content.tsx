...
export function LobbyContent({shortRoomId, gameroom, player}: {shortRoomId:string, gameroom:GameroomAPIResponse, player:PlayerData}) {
  const me = gameroom && gameroom.currentGame && Object.values(gameroom.currentGame).find(p => p.name === player.name);
  const rival = gameroom && gameroom.currentGame && Object.values(gameroom.currentGame).find(p => p.name !== player.name);
  return (
    <>
