...
export function GameroomCard(props:GameroomCardProps) {
    ...
    return (
        <div className={styles["gameroom-card"]}>
            ...
            <button className={`${styles["button"]} ${!props.full ? styles["full"] : ""}`} onClick={props.full ? handleJoinRoom : handleErrorClick}>{props.full ? <DoorOpenIcon size="36"/> : <DoorClosedIcon size="36"/>}</button>
